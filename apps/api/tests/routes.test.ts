...
describe("handleApiRequest with mocked Yahoo fetch", () => {
  ...
  test("returns 200 and ticker-tape quotes when Yahoo v7 quote JSON is valid", async () => {
    globalThis.fetch = mock((url) => {
      ...
      if (u.includes("v7/finance/quote")) {
        const result = SP_TICKER_SYMBOLS.map((symbol, i) => ({
          symbol,
    ...
    expect(body.quotes.length).toBe(SP_TICKER_SYMBOLS.length);
    expect(body.quotes.length).toBeGreaterThanOrEqual(10);
    expect(body.quotes.map((q) => q.symbol)).toEqual([...SP_TICKER_SYMBOLS]);
    expect(body.quotes[0]?.price).toBe(100);
    expect(body.quotes[0]?.changePercent).toBe(1.23);
  });

  test("returns 200 ticker-tape quotes when v7 quote is blocked but v8 chart works", async () => {
    globalThis.fetch = mock((url) => {
      const u = typeof url === "string" ? url : url.toString();
      if (!u.includes("finance.yahoo.com")) {
        return Promise.resolve(new Response("not found", { status: 404 }));
  ...
  test("returns 502 ticker-tape when upstream fails on both v7 and v8", async () => {
    globalThis.fetch = mock((url) => {
      const u = typeof url === "string" ? url : url.toString();
      if (u.includes("v7/finance/quote")) {
        const blocked = { finance: { result: null, error: { code: "Unauthorized", description: "blocked" } } };
      ...
      return Promise.resolve(new Response("upstream down", { status: 500 }));
    }) as unknown as typeof fetch;
