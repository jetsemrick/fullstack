...
import { SP_TAPE_SYMBOLS } from "@stock/shared";
...
describe("handleApiRequest with mocked Yahoo fetch", () => {
  ...
  test("returns 200 and ticker tape quotes when v7 quote JSON is valid", async () => {
    globalThis.fetch = mock((url) => {
      ...
      if (u.includes("v7/finance/quote")) {
        const result = SP_TAPE_SYMBOLS.map((symbol, i) => ({
          symbol,
          shortName: `${symbol} Inc`,
          regularMarketPrice: 100 + i,
          regularMarketChangePercent: i % 2 === 0 ? 1.5 : -0.75,
          marketState: "REGULAR",
        }));
        return Promise.resolve(
    ...
    expect(body.quotes.length).toBe(SP_TAPE_SYMBOLS.length);
    expect(body.quotes.map((q) => q.symbol)).toEqual([...SP_TAPE_SYMBOLS]);
    expect(body.quotes[0]?.price).toBe(100);
  });

  test("returns 200 ticker tape via v8 chart fallback when v7 quote is blocked", async () => {
    globalThis.fetch = mock((url) => {
      const u = typeof url === "string" ? url : url.toString();
      if (!u.includes("finance.yahoo.com")) {
        return Promise.resolve(new Response("not found", { status: 404 }));
  ...
  test("returns 502 when upstream ticker quotes fail", async () => {
    globalThis.fetch = mock(() =>
      Promise.resolve(new Response("blocked", { status: 429 })),
    ) as unknown as typeof fetch;
