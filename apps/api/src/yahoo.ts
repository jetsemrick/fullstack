...
export function parseResult(body: unknown): YahooParseResult {
  if (typeof body !== "object" || body === null) {
    return { errorMessage: "Invalid JSON", points: [], currency: null, openPrice: null, lastPrice: null, symbol: null };
  }
  const chart = (body as Record<string, unknown>).chart;
  if (typeof chart !== "object" || chart === null) {
    return { errorMessage: "Missing chart", points: [], currency: null, openPrice: null, lastPrice: null, symbol: null };
  }
  ...
  if (!Array.isArray(result) || result[0] === undefined) {
    return { errorMessage: "No data for symbol", points: [], currency: null, openPrice: null, lastPrice: null, symbol: null };
  }
  ...
  const openPrice =
    typeof meta === "object" && meta !== null
      ? pickNumber((meta as { regularMarketOpen?: unknown }).regularMarketOpen)
      : null;
  const symbol =
  ...
  if (!Array.isArray(timestamps) || timestamps.length === 0) {
    return { errorMessage: "No series data", points: [], currency, openPrice, lastPrice, symbol };
  }
  ...
  if (!quote || !Array.isArray(quote.close) || quote.close.length !== timestamps.length) {
    return { errorMessage: "Malformed quote data", points: [], currency, openPrice, lastPrice, symbol };
  }
  ...
  for (let i = 0; i < timestamps.length; i++) {
    ...
    const open = quote.open && i < quote.open.length ? pickNumber(quote.open[i]) : null;
    ...
    points.push({ timestamp: ts, open, close, volume });
  }
  if (points.length === 0) {
    return { errorMessage: "No price points", points: [], currency, openPrice, lastPrice, symbol };
  }
  return { errorMessage: null, points, currency, openPrice, lastPrice, symbol };
}
