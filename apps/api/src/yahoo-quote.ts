import type { MarketQuote } from "@stock/shared";
...
function parseQuoteItem(raw: unknown): MarketQuote | null {
...
export function parseQuoteResponse(
  body: unknown,
  orderedSymbols: readonly string[] = MAJOR_INDEX_SYMBOLS,
): YahooQuoteAggregate {
...
function parseQuoteFromChartBody(body: unknown): (MarketQuote & { marketState: string | null }) | null {
...
async function fetchQuotesViaChart(symbols: readonly string[]): Promise<YahooQuoteAggregate> {
  ...
  const tasks = symbols.map(async (symbol) => {
    ...
    const row = parseQuoteFromChartBody(json);
...
async function fetchQuotes(symbols: readonly string[], emptyMessage: string): Promise<YahooQuoteAggregate> {
  ...
  return {
    errorMessage: v7.errorMessage ?? viaChart.errorMessage ?? emptyMessage,
    marketState: null,
    indexes: [],
  };
}
...
export async function fetchMajorIndexQuotes(): Promise<YahooQuoteAggregate> {
  return fetchQuotes(MAJOR_INDEX_SYMBOLS, "No benchmark quotes");
}

export async function fetchTickerTapeQuotes(): Promise<YahooQuoteAggregate> {
  return fetchQuotes(TICKER_TAPE_SYMBOLS, "No ticker tape quotes");
}
