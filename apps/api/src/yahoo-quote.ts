...
export function parseQuoteResponse(
  body: unknown,
  orderBy: readonly string[] = MAJOR_INDEX_SYMBOLS,
): YahooQuoteAggregate {
...
/**
 * Fetch a batch of Yahoo quotes for the given symbols, preserving input order.
 * v7 aggregate quote is often blocked, so fall back to v8 chart meta per symbol
 * (same pathway as `/api/prices`).
 */
export async function fetchQuoteBatch(symbols: readonly string[]): Promise<YahooQuoteAggregate> {
...
/** Benchmark index quotes (S&P 500, Dow, Nasdaq) for the market context strip. */
export function fetchMajorIndexQuotes(): Promise<YahooQuoteAggregate> {
  return fetchQuoteBatch(MAJOR_INDEX_SYMBOLS);
}

/** Curated large-cap S&P quotes for the scrolling ticker tape. */
export function fetchTickerTapeQuotes(): Promise<YahooQuoteAggregate> {
  return fetchQuoteBatch(SP_TICKER_TAPE_SYMBOLS);
}
