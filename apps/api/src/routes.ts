...
export async function handleApiRequest(req: Request): Promise<Response> {
  ...
  if (url.pathname === "/api/ticker-tape" && req.method === "GET") {
    try {
      const yahoo = await fetchTickerTapeQuotes();
      if (yahoo.errorMessage || yahoo.indexes.length === 0) {
        return jsonResponse(
          errBody(yahoo.errorMessage ?? "No ticker tape quotes", "UPSTREAM"),
          { status: 502, headers: corsHeaders() },
        );
      }
      const body: TickerTapeResponse = { quotes: yahoo.indexes };
      return jsonResponse(body, { status: 200, headers: corsHeaders() });
    } catch (e) {
      const msg = e instanceof Error ? e.message : "Unknown error";
      return jsonResponse(errBody("Failed to load ticker tape", "INTERNAL", msg), {
        status: 500,
        headers: corsHeaders(),
      });
    }
