import type { ApiErrorBody, GetPricesResponse, MarketContextResponse, TickerTapeResponse } from "@stock/shared";
import { DEFAULT_TICKER, SP_TAPE_SYMBOLS } from "@stock/shared";
import { fetchYahooChart } from "./yahoo";
import { fetchMajorIndexQuotes, fetchQuotes } from "./yahoo-quote";
...
export async function handleApiRequest(req: Request): Promise<Response> {
  ...
  if (url.pathname === "/api/ticker-tape" && req.method === "GET") {
    try {
      const y = await fetchQuotes(SP_TAPE_SYMBOLS);
      if (y.errorMessage || y.indexes.length === 0) {
        return jsonResponse(
      ...
      const body: TickerTapeResponse = { quotes: y.indexes };
      return jsonResponse(body, { status: 200, headers: corsHeaders() });
    } catch (e) {
      const msg = e instanceof Error ? e.message : "Unknown error";
