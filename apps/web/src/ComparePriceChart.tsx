...
import type { PriceChartVariant } from "./PriceChart";
import {
...
function spanCalendarDays(rows: CompareChartRow[]): number {
...
export function ComparePriceChart({
  datasets,
  variant = "daily",
}: {
  ...
  const { rows, tickers } = useMemo(
    () => buildCompareChartRows(datasets.map((d) => ({ ticker: d.ticker, series: d.series }))),
    [datasets],
  );
  ...
  return (
    <div
      role="img"
      aria-label={`Compare chart for ${tickers.join(", ")}`}
        ...
        <LineChart data={rows} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
          ...
          <Tooltip
            ...
            labelFormatter={(_, payload) => {
              ...
              if (typeof t === "number") return formatTooltipWhen(t, variant, spanDays);
              return "";
            }}
          ...
          {tickers.map((ticker, i) => (
            <Line
              ...
              dataKey={ticker}
              name={ticker}
              stroke={COMPARE_SERIES_COLORS[i % COMPARE_SERIES_COLORS.length]}
              strokeWidth={2.5}
              dot={false}
              connectNulls
