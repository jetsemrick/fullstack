...
export function PriceChart({ data, variant = "daily" }: { data: GetPricesResponse; variant?: PriceChartVariant }) {
  ...
  return (
    <div role="img" aria-label={hasVolume ? "Price over time line chart with volume bars" : "Price over time line chart"} style={{ width: "100%", height: "100%" }}>
        ...
        <ComposedChart data={rows} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
          ...
          {hasVolume && (
            <Legend
              ...
              align="right"
              iconType="plainline"
              wrapperStyle={{ color: "var(--fg-muted)", fontSize: 12, paddingBottom: 8 }}
            />
          )}
          {hasVolume && (
            <Bar
